...
function seleccionarLibro(key, ruta) {
  ...
  // CAMBIO CLAVE: Guardamos el ID del libro y reiniciamos el capítulo a 1 en el almacenamiento
  localStorage.setItem('ultimoLibroId', key);
  localStorage.setItem('ultimoCapituloNum', 1);
...
function seleccionarCapitulo(cap) {
  ...
  // CAMBIO CLAVE: Guardamos el número de capítulo seleccionado en el navegador
  localStorage.setItem('ultimoCapituloNum', capituloActualNum);
...
function asignarEventosViajeReferencia() {
  ...
  enlacesDestino.forEach(bloque => {
    ...
    bloque.addEventListener('click', () => {
      ...
      // 1. Actualizamos el estado global en memoria
      idLibroActual = libroIdDestino;
      rutaLibroActual = rutaDestino;
      capituloActualNum = capDestino;

      // 2. CAMBIO CLAVE: Guardamos en el localStorage el destino del viaje del paralelo
      localStorage.setItem('ultimoLibroId', libroIdDestino);
      localStorage.setItem('ultimoCapituloNum', capDestino);

      // 3. Cargamos las Escrituras del destino
...
/* ==========================================================================
   5. INICIALIZACIÓN CORREGIDA CON PERSISTENCIA
   ========================================================================== */
document.addEventListener('DOMContentLoaded', () => {
  ...
  // D. Carga de referencias cruzadas y arranque inteligente
  fetch('src/js/paralelos.json')
    .then(res => res.json())
    .then(data => {
      mapaEnlacesParalelos = data;
      
      // CAMBIO CLAVE: Intentamos recuperar lo que el usuario estaba leyendo antes de refrescar
      const libroGuardado = localStorage.getItem('ultimoLibroId');
      const capituloGuardado = localStorage.getItem('ultimoCapituloNum');

      // Si existía algo guardado, lo usamos; si no, caemos en Génesis por defecto
      idLibroActual = libroGuardado ? libroGuardado : "01_gn";
      capituloActualNum = capituloGuardado ? parseInt(capituloGuardado, 10) : 1;
      
      // Buscamos la ruta correspondiente en el diccionario basándonos en el ID
      if (indiceLibrosRutas[idLibroActual]) {
        rutaLibroActual = indiceLibrosRutas[idLibroActual].ruta;
      } else {
        rutaLibroActual = "src/libros/01_gn.json";
      }
      
      // Cargamos el libro y capítulo exacto donde se quedó
      cargarLibroYCapitulo(rutaLibroActual, capituloActualNum);
    })
    .catch(err => {
      console.error("Error al cargar paralelos de inicio:", err);
      // Fallback de seguridad usando las variables actuales o Génesis por defecto
      const libroGuardado = localStorage.getItem('ultimoLibroId') || "01_gn";
      const capituloGuardado = localStorage.getItem('ultimoCapituloNum') || 1;
      const rutaFallback = indiceLibrosRutas[libroGuardado] ? indiceLibrosRutas[libroGuardado].ruta : "src/libros/01_gn.json";
      
      cargarLibroYCapitulo(rutaFallback, parseInt(capituloGuardado, 10));
    });
});
